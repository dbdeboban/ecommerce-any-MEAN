...
export class CartComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.handler = StripeCheckout.configure({
      ...
      token: async stripeToken => {
        ...
        try {
          const data = await this.rest.post('api/payment', {
            totalPrice: this.cartTotal,
            products,
            stripeToken
          });
          data['success']
            ? (this.data.clearCart(), this.toastr.success('Purcahse Successful.'))
  ...
  validate() {
    if(!this.quantities.every(data => data > 0)){
      this.toastr.warning('quantity cannot be less than one');
    }
    else if(!localStorage.getItem('token')){
      this.router.navigate(['/login'])
        .then(() => {
          this.toastr.warning('you need to login before making a purchase');
        });
    }
